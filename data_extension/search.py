        ...
        for idi, i in enumerate(tables_connected):
            ...
            for j in i:
                ...
                for col in self.real_tables[tname].columns:
                    #if col == "Unnamed: 0" or "index" in col:
                    #    continue
...
#        other_key.remove(key)
    ...
    def search_similar_tables_threshold2(self, query, beta, k, theta, thres_key_cache, thres_key_prune, tflag):
        ...
        start_time = timeit.default_timer()
        # Do mapping
        meta_mapping = SM_test.mapping_naive_tables(self.query, query_col, self.schema_element_sample, self.schema_element_dtype, tflag)
        end_time= timeit.default_timer()
        time1 += end_time - start_time
        print(meta_mapping)
...
class SearchProv:
    ...
    def __init__(self):
        self.model_address = '/Users/yizhang/PycharmProjects/sig_demo/nb_data_extension/notebook_data_extension/data_extension/similar_table_model'
        self.Graphs = self.__read_models()
...
def search_tables(search_test, var_df, mode, code, var_name):
    ...
    if mode == 0:
        ...
        print(mode)
        if mode == 1:
            tables = search_test.search_similar_tables_threshold2(query_table, 0.5, 10, 1, 0.9, 0.2, True)
            #tables = search_test.search_similar_tables_threshold2(query_table, 10, 0.5, 5, 1, 0.9, 0.2, True, 10)
        elif mode == 2:
            tables = search_test.search_joinable_tables_threshold2(query_table, 0.1, 10, 1.5, 0.9, 0.2, True)
        elif mode == 3:
